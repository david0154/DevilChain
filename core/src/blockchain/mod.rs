//! DevilChain Blockchain Core — burn cap + auto liquidity timelock support

use sha2::{Sha256, Digest};
use serde::{Serialize, Deserialize};
use std::collections::HashMap;
use crate::tokenomics::{
    split_fee_with_state, DEV_WALLET, BURN_ADDRESS, LIQUIDITY_LOCK_VAULT, MARKETING_WALLET,
...
impl Transaction {
    ...
    pub fn gas_fee_sufficient(&self) -> bool {
        self.gas_fee >= compute_gas(self.amount, self.tx_type)
    }
...
impl Blockchain {
    fn genesis(&mut self) {
        let mut g = Block {
            ...
            total_fees: 0,
            ai_score: 100,
            dao_signature: "genesis_dao".into(),
        };
    ...
    pub fn add_block(&mut self, mut block: Block) -> Result<(), &'static str> {
        ...
        let prev_hash = self.chain.last().map(|b| b.block_hash.clone()).unwrap_or_else(|| "0".repeat(64));
        if block.previous_hash != prev_hash { return Err("Previous hash mismatch"); }
        if block.merkle_root != Block::compute_merkle_root(&block.transactions) { return Err("Merkle root mismatch"); }
        ...
        let burn_stopped = self.supply.should_stop_burn();
        let split = split_fee_with_state(total_fees, burn_stopped);

        self.ledger.credit(&block.validator, split.miner);
        self.ledger.credit(DEV_WALLET, split.dev);

        let actual_burn = self.supply.burn(split.burn);
        if actual_burn > 0 {
            self.ledger.credit(BURN_ADDRESS, actual_burn);
        }
        let burn_leftover = split.burn.saturating_sub(actual_burn);
        if burn_leftover > 0 {
            self.ledger.credit(DEV_WALLET, burn_leftover / 2);
            self.ledger.credit(LIQUIDITY_LOCK_VAULT, burn_leftover - (burn_leftover / 2));
        }

        self.ledger.credit(LIQUIDITY_LOCK_VAULT, split.liquidity);
        self.ledger.credit(MARKETING_WALLET, split.marketing);

        let liquidity_balance = self.ledger.balance(LIQUIDITY_LOCK_VAULT);
        let now_secs = block.timestamp;
        let _ = self.supply.auto_lock_liquidity_if_needed(liquidity_balance, now_secs);
        let _ = self.supply.unlock_matured_timelocks(now_secs);
