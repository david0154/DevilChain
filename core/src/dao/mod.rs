...
//! Real proposal + voting + execution logic
//!
...
pub struct Proposal {
    ...
    pub votes_yes:   Amount,  // weighted by stake
    pub votes_no:    Amount,
    pub voters:      HashMap<String, bool>,  // addr → yes/no
    pub status:      ProposalStatus,
    pub created_at:  u64,
    pub expires_at:  u64,   // block height
    pub quorum:      Amount, // minimum votes required
}
...
impl DaoGovernance {
    pub fn create_proposal(&mut self, title: String, description: String,
                           ...
                           current_block: u64, quorum: Amount) -> u64
    {
        let id = self.next_id;
        self.next_id += 1;
        self.proposals.push(Proposal {
            ...
            expires_at: current_block + 10_000, // ~8h at 3s blocks
            quorum,
        });
        id
    }

    pub fn vote(&mut self, proposal_id: u64, voter: String,
                yes: bool, voting_power: Amount) -> Result<(), &'static str>
    {
        ...
        if yes { p.votes_yes += voting_power; }
        else   { p.votes_no  += voting_power; }
        Ok(())
    }
